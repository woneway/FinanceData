...
| `tool_get_daily_basic` | daily_basic | 日频基本面（PE/PB/市值/换手率/量比），tencent(腾讯API) |
...
| `tool_get_lhb_detail` | lhb | 龙虎榜每日上榜详情（按日期范围），akshare(东财)+tushare |
| `tool_get_lhb_inst_detail` | lhb | 龙虎榜机构买卖每日统计，akshare(东财) |
...
| `tool_get_north_stock_hold` | north_flow | 北向资金持股明细，akshare(东财)+tushare |
