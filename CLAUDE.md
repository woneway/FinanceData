...
└── mcp/
    └── server.py       # MCP server：16 个 tool 定义，调用 service 层
...
| `tool_get_sector_capital_flow` | 依赖 stock_sector_fund_flow_rank |

## Provider 优先级

`akshare`（无需 token）→ `tushare`（需 `TUSHARE_TOKEN`）→ `xueqiu`（海外可达，9 个接口；实时行情/个股信息/资金流向/财务摘要/分红/融资融券明细无需认证，K线需登录 cookie — 自动从浏览器提取或手动设置 `XUEQIU_COOKIE`）
