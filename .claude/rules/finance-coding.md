...
新增或修改 provider 时，禁止假设字段名和数据结构。真实 API 返回是唯一事实来源。

**强制步骤（缺一不可）：**
1. 获取官方文档，提取入参表和出参表
2. 真实调用 API，`print(df.columns.tolist())` 确认字段名
3. `r.get("xxx")` 必须与第 2 步 columns 完全一致
4. 打印样本行，验证数值量级和单位（参照"数据单位"章节）
5. `invoke_tool_spec()` 端到端调用，关键字段非零
