...
@pytest.fixture
def mock_sina_index_df():
    """新浪源返回 date/open/close/high/low/volume（无 amount）"""
    return pd.DataFrame([
        {"date": "2023-12-29", "open": 3050.0, "close": 3080.0, "high": 3090.0, "low": 3040.0, "volume": 5e9},
        {"date": "2024-01-02", "open": 3090.0, "close": 3100.0, "high": 3110.0, "low": 3085.0, "volume": 6e9},
    ])


def test_get_index_history_returns_data_result(history_provider, mock_sina_index_df):
    with patch("finance_data.provider.akshare.index.history.ak.stock_zh_index_daily",
               return_value=mock_sina_index_df):
...
def test_get_index_history_has_amount(history_provider, mock_sina_index_df):
    with patch("finance_data.provider.akshare.index.history.ak.stock_zh_index_daily",
               return_value=mock_sina_index_df):
        result = history_provider.get_index_history("000001.SH", start="20240101", end="20240102")
    bar = result.data[0]
    assert bar["amount"] == 0.0  # 新浪源不提供成交额
    assert bar["volume"] == 6e9


def test_get_index_history_pct_chg_calculated(history_provider, mock_sina_index_df):
    with patch("finance_data.provider.akshare.index.history.ak.stock_zh_index_daily",
               return_value=mock_sina_index_df):
        result = history_provider.get_index_history("000001.SH", start="20240101", end="20240102")
    bar = result.data[0]
