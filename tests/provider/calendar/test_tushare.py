...
def test_get_trade_calendar_returns_data_result(mock_pro, mock_cal_df):
    ...
    with patch("finance_data.provider.tushare.calendar.history.get_pro", return_value=mock_pro):
        result = TushareTradeCalendar().get_trade_calendar_history(start="20240101", end="20240102")
...
def test_get_trade_calendar_empty_raises(mock_pro):
    ...
    with patch("finance_data.provider.tushare.calendar.history.get_pro", return_value=mock_pro):
        with pytest.raises(DataFetchError) as exc:
            TushareTradeCalendar().get_trade_calendar_history(start="20240101", end="20240101")
