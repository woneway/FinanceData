...
系统 MUST 默认启用 DuckDB 本地缓存层；调用方 MUST 能通过单一开关显式关闭缓存（例如开发或测试环境）。当缓存关闭时，所有读请求 MUST 直接落回上游 API，不查任何本地缓存。
...
#### Scenario: 显式关闭
- **WHEN** 维护方设置缓存关闭开关（当前实现：`FINANCE_DATA_CACHE=0`）
- **THEN** 所有 provider MUST 直接调用上游 API
- **AND** 测试环境 `tests/conftest.py` MUST 默认关闭缓存以避免 mock 被绕过
