...
TOOL_SPEC_REGISTRY: "OrderedDict[str, ToolSpec]" = OrderedDict(
    ...
    for spec in [
        ...
        ToolSpec(
            name="tool_get_sector_list",
            description="获取行业板块列表（含实时行情概览）",
            domain="sector",
            params=(),
            return_fields=("name", "code", "pct_chg", "market_cap", "up_count", "down_count"),
            service=_service("finance_data.service.sector", "sector_list", "get_sector_list"),
            providers=(
                _provider("akshare", "finance_data.provider.akshare.sector.list:AkshareSectorList", "get_sector_list"),
            ),
            probe=_probe({}, required_fields=("name", "code")),
            metadata=_meta(entity="sector", scope="realtime", data_freshness="realtime", update_timing="T+0", supports_history=False, source="akshare", source_priority="akshare", api_name="stock_board_industry_name_em", primary_key="code"),
        ),
        ToolSpec(
            name="tool_get_sector_member",
            description="获取行业板块成分股列表",
            domain="sector",
            params=(
                _param("sector_name", required=True, description="板块名称", example="银行", aliases=("symbol",)),
            ),
            return_fields=("symbol", "name", "price", "pct_chg", "volume", "amount"),
            service=_service("finance_data.service.sector", "sector_member", "get_sector_member"),
            providers=(
                _provider("akshare", "finance_data.provider.akshare.sector.member:AkshareSectorMember", "get_sector_member"),
            ),
            probe=_probe({"symbol": "银行"}, required_fields=("symbol",)),
            metadata=_meta(entity="stock", scope="realtime", data_freshness="realtime", update_timing="T+0", supports_history=False, source="akshare", source_priority="akshare", api_name="stock_board_industry_cons_em", primary_key="symbol"),
        ),
        ToolSpec(
            name="tool_get_sector_history",
            description="获取行业板块历史K线行情",
            domain="sector",
            params=(
                _param("sector_name", required=True, description="板块名称", example="银行", aliases=("symbol",)),
                _param("start_date", required=False, default="20240101", description="开始日期 YYYYMMDD", example="20240301"),
                _param("end_date", required=False, default="", description="结束日期 YYYYMMDD", example="20240409"),
                _param("period", required=False, default="日k", description="周期", example="日k"),
            ),
            return_fields=("date", "open", "close", "high", "low", "volume", "amount", "pct_chg"),
            service=_service("finance_data.service.sector", "sector_history", "get_sector_history"),
            providers=(
                _provider("akshare", "finance_data.provider.akshare.sector.history:AkshareSectorHistory", "get_sector_history"),
            ),
            probe=_probe({"symbol": "银行", "start_date": "$RECENT-30", "end_date": "$RECENT"}, required_fields=("date", "close")),
            metadata=_meta(entity="sector", scope="historical", data_freshness="end_of_day", update_timing="T+1_16:00", supports_history=True, history_start="20150101", source="akshare", source_priority="akshare", api_name="stock_board_industry_hist_em", primary_key="date"),
        ),
        ToolSpec(
